...
if [ "$?" = 0 ]; then
  cd "$DIR/tomcat/bin"
  CATALINA_OPTS="-Xms{{di_server_xms}} -Xmx{{di_server_xmx}} -XX:MaxPermSize=256m -Dsun.rmi.dgc.client.gcInterval=3600000 -Dsun.rmi.dgc.server.gcInterval=3600000 -Dfile.encoding=utf8 -DDI_HOME=\"$DI_HOME\" -DHADOOP_USER_NAME=$HADOOP_USER_NAME"
